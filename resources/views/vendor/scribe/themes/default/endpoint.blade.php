...
<blockquote>Пример запроса:</blockquote>
...
@if($endpoint->isGet() || $endpoint->hasResponses())
    @foreach($endpoint->responses as $response)
        <blockquote>
            <p>Пример запроса ({{$response->description ?: $response->status}}):</p>
        </blockquote>
...
<span id="execution-error-{{ $endpoint->endpointId() }}" hidden>
    <blockquote>HTTP ответ ошибка:</blockquote>
...
<form id="form-{{ $endpoint->endpointId() }}" data-method="{{ $endpoint->httpMethods[0] }}"
    ...
    @if(count($endpoint->urlParameters))
        <h4 class="fancy-heading-panel"><b>Параметры запроса</b></h4>
